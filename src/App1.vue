...
async function initMap() {
  ...
  let center = new TMap.LatLng(31.839301, 117.110363, 0); // 设置中心点坐标
  ...
  glCustomLayer = new TMap.GLCustomLayer({
    ...
    init: (gl) => {
      ...
      // 立方体纹理贴图
      // 前面
      const textureFront = new THREE.TextureLoader().load(new URL('./assets/textures/front.png', import.meta.url), function (texture) {
        texture.wrapS = THREE.RepeatWrapping;
        texture.wrapT = THREE.RepeatWrapping;
        texture.offset.set(0, 0.5);
        texture.repeat.set(0.0001, 0.0001);
      });
      textureFront.flipY = false;

      // 根据经纬度轨迹绘制不规则集合体
      let coordsArr = [
        // [31.839254, 117.099784], // 西北
        // [31.839392, 117.099784], // 东北
        // [31.839392, 117.099433], // 东南（重复的坐标，这里应该是另一个坐标）
        // [31.839254, 117.099433], // 西南

        [31.839301, 117.110363], // 西北
        [31.839158, 117.110374], // 东北
        [31.83915, 117.110486], // 东南
        [31.83911, 117.110489], // 西南

        [31.839114, 117.110752], // 西北
        [31.839147, 117.110758], // 东北
        [31.839158, 117.110886], // 东南
        [31.839308, 117.110875], // 西南

        [31.839302, 117.110773], // 西北
        [31.839348, 117.110772], // 东北
        [31.839348, 117.110675], // 东南
        [31.839303, 117.110675], // 西南

        [31.839303, 117.110591], // 西北
        [31.839348, 117.110591], // 东北
        [31.839348, 117.110486], // 东南
        [31.839301, 117.110482], // 西南
      ...
      for (let i = 0; i < 9; i++) {
        ...
        //经纬度坐标转二维向量
        let coordsArrVector = coordsArr.map((item) => {
          return map.glLatLngToPosition(new TMap.LatLng(item[0], item[1], 0));
        });

        console.log(coordsArrVector, 'coordsArrVector');

        let shape = new THREE.Shape();
        shape.moveTo(coordsArrVector[0].x, coordsArrVector[0].y);
        for (let i = 1; i < coordsArrVector.length; i++) {
          shape.lineTo(coordsArrVector[i].x, coordsArrVector[i].y);
        }
        shape.lineTo(coordsArrVector[0].x, coordsArrVector[0].y);
        ...
        // let material = new THREE.MeshBasicMaterial({ color: i % 2 == 0 ? 0x00ff00 : 0xffffff });
        // let material = [

        //   new THREE.MeshBasicMaterial({ color: i % 2 == 0 ? 0x00ff00 : 0xffffff }),

        //   new THREE.MeshBasicMaterial({ map: textureFront }),
        //   new THREE.MeshBasicMaterial({ color: i % 2 == 0 ? 0x00ff00 : 0xffffff })
        // ];
        let material = i % 2 == 0 ? new THREE.MeshBasicMaterial({ map: textureFront }): new THREE.MeshBasicMaterial({ color: 0xffffff });
...
function createCube(coords, scene, group, map) {
  ...
  for (let i = 0; i < len; i++) {
    let material = [
      ...
      // 下面
      new THREE.MeshBasicMaterial({ color: 0xffffff })]; // 红色材质
